...
import random
import math
from collections import deque        # ← missing import
...
class LearnedLocalPlanner(LocalPlanner):
    ...
    def set_relative_trajectory(self, rel_actions, dt=0.1):
        """
        rel_actions : iterable of (delta_speed  [m/s],
                                   delta_yaw    [deg])
        """
        plan, target_speeds = self._build_plan(rel_actions, dt)
        self._future_speeds = deque(target_speeds, maxlen=len(target_speeds))
    ...
    def run_step(self, debug=False):
        if hasattr(self, "_future_speeds") and self._future_speeds:
            self._target_speed = self._future_speeds.popleft() * 3.6   # m/s → km/h
